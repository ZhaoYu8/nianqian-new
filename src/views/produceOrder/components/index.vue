  ...
  <div>
    <el-row :gutter="20">
      <el-form ref="form" :model="formOptions" :rules="rules" label-width="120px">
        <el-col :span="6">
          <el-form-item label="加工商：" prop="customer_id">
            <page v-model="formOptions.customer_id" size="small" placeholder="请选择" :data="formData.producer_customers" @change="change"></page>
          </el-form-item>
        </el-col>
        <el-col :span="6">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        order_id: [{ required: true, message: "请选择相关业务单", trigger: "change" }]
      },
      tableData: [],
      defaultData: {
  ...
  methods: {
    ...
    setOptions() {
      ...
      this.change();
      this.tableData = this.updateData.products_attributes.map((r) => {
        return {
          ...r,
          ...{
      ...
      this.fileList = this.updateData.order_files_attributes.map((r) => {
        return {
          name: r.file_name
        };
      });
  ...
  mounted() {
    this.formData = {
      producer_customers: this.data.producer_customers, // 下单客户
